...
- `CAPSULES` - Time Capsules feature settings
...
| THROTTLING.RATE_LIMIT.BAN_DURATION | SHUGUR_RELAY_THROTTLING_RATE_LIMIT_BAN_DURATION | Ban duration for rate limit violations. | 5m |
...
Time Capsules feature configuration for NIP-XX timelock encryption.

| YAML Key | Environment Variable | Description | Default |
| :--- | :--- | :--- | :--- |
| ENABLED | SHUGUR_CAPSULES_ENABLED | Whether to enable Time Capsules feature. | true |
| MAX_WITNESSES | SHUGUR_CAPSULES_MAX_WITNESSES | Maximum witnesses for threshold schemes (legacy, not used for drand). | 9 |

### Time Capsules Configuration

Time Capsules use drand randomness beacon network for timelock encryption:

- **Public Capsules**: Time-locked messages (kind 1041) that anyone can decrypt after unlock time
- **Private Capsules**: Gift-wrapped time capsules using NIP-59 for targeted delivery
- **Drand Integration**: Uses real drand networks (Quicknet, Mainnet) for decentralized timelock
...
CAPSULES:
  ENABLED: true
  MAX_WITNESSES: 9  # Legacy setting, kept for compatibility
```
...
# Time Capsules configuration
export SHUGUR_CAPSULES_ENABLED="true"
export SHUGUR_CAPSULES_MAX_WITNESSES="9"

# Rate limiting enhancements
export SHUGUR_RELAY_THROTTLING_RATE_LIMIT_BAN_DURATION="5m"
export SHUGUR_RELAY_THROTTLING_RATE_LIMIT_MAX_BAN_DURATION="24h"
```
